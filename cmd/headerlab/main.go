// Package main renders header-design experiments for the till TUI.
package main
...
// buildPalette returns the default till-like color palette for previews.
...
func buildVariants() []headerVariant {
	return []headerVariant{
		...
		},
		{
			...
			Render: func(p previewPalette, width int) string {
				logoBox := lipgloss.NewStyle().
					...
					BorderForeground(p.dim).
					Padding(0, 2).
					Render(lipgloss.NewStyle().Bold(true).Foreground(p.fg).Render("TILL"))
				ramp := []string{
					"#5A56E0",
					"#4E68F2",
					"#417EF9",
		...
		},
		{
			...
			Render: func(p previewPalette, width int) string {
				rule := lipgloss.NewStyle().Foreground(p.accent).Render(strings.Repeat("=", max(8, width)))
				title := lipgloss.NewStyle().Bold(true).Foreground(p.fg).Align(lipgloss.Center).Width(width).Render("till")
				path := lipgloss.NewStyle().Foreground(p.muted).Render(truncate("path: "+samplePath, width))
		...
		},
		{
			...
			Render: func(p previewPalette, width int) string {
				top := lipgloss.NewStyle().Bold(true).Foreground(p.fg).Render("till")
				shadow := lipgloss.NewStyle().Foreground(p.dim).Render("  till")
				path := lipgloss.NewStyle().Foreground(p.muted).Render(truncate("path: "+samplePath, width))
...
func renderSheet(p previewPalette, variants []headerVariant, width int) string {
	title := lipgloss.NewStyle().Bold(true).Foreground(p.accent).Render("till header design playground")

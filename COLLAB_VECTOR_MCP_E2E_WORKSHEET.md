...
|---|---|---|---|---|---|---|
| T1-01 | Open edit task form for existing task | `objective`, `acceptance_criteria`, `validation_plan`, `risk_notes`, `blocked_reason` are visible/editable | PENDING_USER | User rerun found the edit full-page screen still clipped its bottom border, kept a wider right gutter than the left, failed to auto-scroll newly focused rows, treated `depends_on`/`blocked_by` differently from modal-only action rows like `due`/`labels`, failed to open resources from `enter`, and still felt architecturally split across info/edit/add/subtask. The remediation wave replaced the split full-page layout path with one measured full-page surface contract, added consistent outer inset budgeting so bordered full-screen surfaces stay on-screen, moved description-editor panel sizing onto the same measured surface contract, kept task/project form viewport state persistent so arrow-key focus auto-scrolls correctly, tracked focused subtask/resource rows for viewport visibility, normalized dependency rows into the same action-row pattern as due/labels, retained resource/subtask/comments rows in the shared task form contract, reset parent-navigation history so `backspace` -> parent and `esc` behave naturally, and re-synced stale task/thread help so it only advertises working actions. | Implementation in `internal/tui/full_page_surface.go`, `internal/tui/model.go`, `internal/tui/thread_mode.go`, `internal/tui/description_editor_mode.go`, `internal/tui/model_test.go`; validations green: `just test-pkg ./internal/tui`, `just check`, `just ci`; QA sign-off recorded in `FX-008` validation row. | Same blocked step remains the next user rerun target; forward collaborative testing stays paused until that rerun. |
...
- Open High findings: `1 (FR-008 ready for user rerun)`
